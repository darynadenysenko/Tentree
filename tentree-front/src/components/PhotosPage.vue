...
export default({
    ...
    data() {
        return {
            ...
            spotName: '', // This will store the name of the spot
            photos: [], // This will store the photos for the selected spot
            loading: true, // This will indicate if the data is still being fetched
            selectedFile: null,  // To hold the selected file for upload
    ...
    methods: {
        fetchPhotos() {
            fetch(`http://localhost:3000/spotphotos/${this.spotId}`)
        ...
        uploadPhoto() {
            ...
            .then(response => response.json())
            .then(data => {
                this.photos.push(data);  // Add the new photo to the UI
        ...
        deletePhoto(photoId) {
            ...
            .then(() => {
                // Remove the photo from the UI after successful deletion
